...
_ztheme_usage() {
  ...
  print '  reset            Restore Catppuccin Mocha for this session'
...
_ztheme_apply() {
  ...
  local name=$1 old_ui=${ZSH_UI_THEME:-catppuccin-mocha} old_fzf=${ZSH_FZF_THEME-}
...
_ztheme_role_sgr() {
  ...
  local theme=$1 role=$2 layer=${3:-bg} depth
  local hex value
...
_ztheme_list() {
  ...
  for name in "${_ZSH_UI_THEME_NAMES[@]}"; do
    ...
    [[ $name == catppuccin-mocha ]] && annotations+='default '
...
_ztheme_current() {
  ...
  print -r -- "UI theme: ${_ZSH_UI_ACTIVE_THEME:-catppuccin-mocha}"
  print -r -- "fzf theme: ${_ZSH_FZF_ACTIVE_THEME:-${_ZSH_UI_ACTIVE_THEME:-catppuccin-mocha}}${fzf_note}"
...
# Inspect or switch the shared dashboard and finder theme for this session.
ztheme() {
  emulate -L zsh
  ...
  case $command in
    ...
    show)
      ...
      theme=${2:-${_ZSH_UI_ACTIVE_THEME:-catppuccin-mocha}}
    ...
    reset)
      (( $# == 1 )) || { _ztheme_usage >&2; return 1; }
      _ztheme_apply catppuccin-mocha
      ;;
    export)
      (( $# == 2 )) || { _ztheme_usage >&2; return 1; }
