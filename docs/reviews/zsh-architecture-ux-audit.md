...
The original audit findings are implemented. Subsequent review reproduced two integration regressions missed by the maintenance checks; their remediation and cross-repository merge dependency are recorded below. Terminal-dependent shaping limits and host/service checks remain documented verification boundaries, not claims of completed manual QA.


### Post-review integration regressions (2026-09-05)

- **Fresh-shell Nix search:** reproduced `upkg search ripgrep --only=nix` failing with `command not found: _npkg_nix` before any `npkg` invocation. The package search adapter now explicitly loads the fixed Nix domain before calling its private adapter. A new fresh-shell test verifies that the adapter starts unloaded, the native search receives the expected arguments, and the public command returns results successfully. The full shell runner passes with 1,268 `ok:` assertions.
- **Website extraction:** reproduced the existing zsh-web extractor failing with `no _zsh_help_register records found`. The companion zsh-web commit `dd31841` on `fix/shell-audit-extractor` reads `lib/command-registry.zsh`, discovers implementations in the split domains, validates action parents, and emits runnable action examples with their parent source paths. It also ignores printed autoload instructions. Generated data contains 53 commands and 70 tips. Website `npm run verify` passes, including lint, type checks, build, and unchanged payload budgets; all 34 desktop/mobile Playwright tests pass. The companion branch is saved in the existing local zsh-web repository without changing its working tree. Isolated extractor regressions cover synchronization, missing modules, mismatched action parents, and autoload prose; CI runs these checks against its shell checkout.
- **Merge coordination:** review the companion website commit together with this shell change. Website CI currently checks the shell default branch; its extraction check cannot pass against the old layout. Verify the pair with `ZSH_CONFIG_DIR=/path/to/shell-checkout npm run verify` before merging, then rerun website CI once the shell audit changes reach the default branch. Ship the website extractor and regenerated data together. Local verification is not a claim that remote CI has run or either repository has merged.
- The separate website UX/UI audit and WSL crash investigation remain outside this change. The website Actions filter only adds access to the new records; responsive wrapping and example-copy checks cover that integration.
