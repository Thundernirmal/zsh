...
`fbr` lists local and remote branches by recent commit and previews the log. A local branch registered to a Git worktree is annotated with that worktree's path; selecting it changes the current shell to that path. This also applies when selecting the corresponding remote branch. Other selections keep the checkout behavior: a remote branch creates a tracking branch when no local branch with the same short name exists.
